

output_path='~/amcdrive/_AZ_Kay/_Master Databases/'
assay_file='~/amcdrive/_AZ_Kay/_Master Databases/drill assays master.csv'
spec_file='~/amcdrive/_AZ_Kay/_Master Databases/spectral master.csv'
alter_file='~/amcdrive/_AZ_Kay/_Master Databases/alteration master.csv'
mineral_file='~/amcdrive/_AZ_Kay/_Master Databases/mineralization master.csv'
sample_file='~/amcdrive/_AZ_Kay/_Master Databases/samples master.csv'
structure_file='~/amcdrive/_AZ_Kay/_Master Databases/structure master.csv'
lith_file='~/amcdrive/_AZ_Kay/_Master Databases/lithology master.csv'
hyp_file='~/amcdrive/_AZ_Kay/_Master Databases/hyp-package samples master.csv'
xrf_file='~/amcdrive/_AZ_Kay/_Master Databases/xrf samples master.csv'

assay_fname=assay_file.split('/')[-1]
spec_fname=spec_file.split('/')[-1]
alter_fname=alter_file.split('/')[-1]
mineral_fname=mineral_file.split('/')[-1]
sample_fname=sample_file.split('/')[-1]
structure_fname=structure_file.split('/')[-1]
lith_fname=lith_file.split('/')[-1]
hyp_fname=hyp_file.split('/')[-1]
xrf_fname=xrf_file.split('/')[-1]

